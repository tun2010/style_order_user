	...
	div.simulator__selector.simulator__selector_design
		div.selector_design__category
			...
			ul(v-if="$parent.selected.gender == 'women'")
				...
				li.buttons(:class="{'buttons-line':$parent.c3Category == 6 && sub=='pants2','buttons-black':$parent.c3Category != 6}" v-on:click="c3CateChanger(6,'pants2',32)"  v-if="courseNos[2] == '012'") SLACKS2
				...
				li.buttons(:class="{'buttons-line':$parent.c3Category == 7 && sub == 'skirt2','buttons-black':$parent.c3Category != 7}" v-on:click="c3CateChanger(7,'skirt2',33)" v-if="courseNos[2] == '013' && courseNos[1] == '013'") SKIRT2
			ul
			...
			div.selector_design_search
				p
					span.design_search__inputbox
						...
						input(type="text" placeholder="デザインNO検索")
					span.buttons.buttons-black.buttons-tatesmall 検索
			transition(name="slide" mode="out-in")
				ul(v-if="disp == 1" key="course" :data-sp="selectedParts")
					li(v-for="(parts,key) in $parent.masters.style" v-if="(parts.pattern != false && parts.pattern.indexOf(''+$parent.c3CategoryKatagami) >= 0 && ((parts.style.indexOf(''+$parent.c3Style) >= 0 && ($parent.selected.gender == 'women' || $parent.c3Category == 2)) || ($parent.selected.gender != 'women' && $parent.c3Category != 2)) && parts.pattern.indexOf(''+selectedParts) >= 0) && (($parent.selected.gender == 'men' && parts.gender.men == 'true') || ($parent.selected.gender == 'women' && parts.gender.women == 'true'))" :data-target="key" v-on:click="partsClicker(key,$event)" :class="{'active':key in selectedSelect}")
						div
							figure
				...
				ul(v-if="selectedCourseInPatterns && disp == 2" key="katagami" class="katagami_list")
					li.clicknot(v-for="(parts,key) in selectedCourseInPatterns" :data-target="key" :class="{'active':parts.pattern_num == selectedSelectKatagami}" v-if="(($parent.selected.gb && parts.gb == '1') || (!$parent.selected.gb && (($parent.selected.gender != 'women' && $parent.c3Category != 2) && $parent.c3Style == parts.style) ||($parent.selected.gender == 'women' || $parent.c3Category == 2))) && (String(parts.factory).indexOf($parent.selectedProductData.factory) > -1 || $parent.debug) && ((kiziwashableCheck && optionKiziwashableCheck(parts)) || kiziwashableCheck == false || $parent.debug)")
						div
							...
							div
								...
								div.selectparts__buttons
									p.buttons.buttons-brown.buttons-small.buttons-next.button-info(v-if="parts.detail_images !== null" v-on:click="ivModalOpen($parent.uploadPass+parts.detail_images,'image')") INFO
		...
		katagamiClicker:function(target,event){
			...
			// }else{
				...
				if(this.$parent.c3Category == "1" || this.$parent.c3Category == "5"){
					Vue.set(this.$parent.selected.sessions.ordersheet,"jacket_dno",this.selectedCourseInPatterns[target].pattern_num);
				}else if((this.$parent.c3Category == "3" && this.sub == "") || (this.$parent.c3Category == "6" && this.sub == "")){
					Vue.set(this.$parent.selected.sessions.ordersheet,"slacks_dno1",this.selectedCourseInPatterns[target].pattern_num);
				}else if((this.$parent.c3Category == "3" && this.sub == "pants2") || (this.$parent.c3Category == "6" && this.sub == "pants2")){
					Vue.set(this.$parent.selected.sessions.ordersheet,"slacks_dno2",this.selectedCourseInPatterns[target].pattern_num);
				}else if(this.$parent.c3Category == "2"){
					Vue.set(this.$parent.selected.sessions.ordersheet,"best_dno",this.selectedCourseInPatterns[target].pattern_num);
				}
				else if(this.$parent.c3Category == "7" && this.sub == ""){
					Vue.set(this.$parent.selected.sessions.ordersheet,"wo_sk_d1",this.selectedCourseInPatterns[target].pattern_num);
