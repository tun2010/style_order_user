		...
		div.product__list_selectparts.product__list_selectparts_katagami
			...
			transition(name="slide" mode="out-in")
				...
				ul(v-if="selectedCourseInPatterns && disp == 2 && sorters == false" key="katagami" class="katagami_list")
					li.clicknot(v-for="(parts,key) in selectedCourseInPatterns" :data-target="key" :data-patternid="parts.pattern_id" :class="{'active':parts.pattern_num == selectedSelectKatagami}" v-if="((($parent.selected.gb && parts.gb == '1') || (!$parent.selected.gb && (($parent.selected.gender != 'women' && $parent.c3Category != 2) && $parent.c3Style == parts.style) ||($parent.selected.gender == 'women' || $parent.c3Category == 2))) && (String(parts.factory).indexOf($parent.selectedProductData.factory) > -1 || $parent.debug) && ((kiziwashableCheck && optionKiziwashableCheck(parts)) || kiziwashableCheck == false || $parent.debug)) && newChecker(parts)")
						div
							...
							div
								...
								div.selectparts__buttons
									...
									p.buttons.buttons-brown.buttons-small.buttons-next(v-if="parts.youtube !== null" v-on:click="katagamiClicker(key,$event,'',parts.pattern_num,parts.pattern_id)") SIZE
				//検索用の、カテゴリ内すべての型紙を並べるリスト
				ul(v-if="disp == 2 && sorters" key="katagami2" class="katagami_list")
					li.clicknot(v-for="(parts,key) in categoryZen()" :data-target="key" :data-patternid="parts.pattern_id" :class="{'active':parts.pattern_num == selectedSelectKatagami}" v-if="((($parent.selected.gb && parts.gb == '1') || (!$parent.selected.gb && (($parent.selected.gender != 'women' && $parent.c3Category != 2) && $parent.c3Style == parts.style) ||($parent.selected.gender == 'women' || $parent.c3Category == 2))) && (String(parts.factory).indexOf($parent.selectedProductData.factory) > -1 || $parent.debug) && ((kiziwashableCheck && optionKiziwashableCheck(parts)) || kiziwashableCheck == false || $parent.debug)) && sorter2run(parts) && newChecker(parts)")
						div
							div.katagami_selector_info
								p.katagami_num {{apiNameGetter(parts,henkankun($parent.c3Category))}}
								p.katagami_size(v-if="parts.pattern_num == selectedSelectKatagami && targetSize != ''") 
							...
							div
								...
								div.selectparts__buttons
									...
									// p.buttons.buttons-brown.buttons-small.buttons-next.button-info(v-if="parts.detail_text !== null && parts.detail_text != ''" v-on:click="katagamiInfoOpen(parts.detail_text)") INFO
									p.buttons.buttons-brown.buttons-small.buttons-next(v-if="parts.youtube !== null" v-on:click="katagamiClicker(key,$event,parts.style_option,parts.pattern_num,parts.pattern_id)") SIZE
...
module.exports = {
	...
	methods:{
		...
		katagamiClicker:function(target,event,styleno,pattern_nums,pattern_ids){
			if(this.sorters){
			...
			// this.$parent.optionReseter();
				...
				if(zenKatagami[target].gb == "1"){
					thista.$parent.selected.gb = true;
					//firstcheckersを設定しなおす
					if(thista.$parent.firstCheckers.ResetedFlg == false || this.$parent.tantouFlg == true){
